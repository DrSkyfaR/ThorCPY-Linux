...
class CTkUI:
    ...
    def __init__(self, launcher):
        ...
        # Background threads (wireless/scan) post status updates here; drained on
        # the Tk thread by _update_loop (Tcl is not thread-safe).
        ...
        # Making sure titlebar is dark (no-op off Windows)
        self.window.update_idletasks()
    ...
    def _build_ui(self):
        ...
        # File browser (device <-> local PC)
        ctk.CTkButton(
            self._scroll,
            text="File Browser",
            command=self._on_file_browser,
            fg_color=PANEL_COLOUR,
            hover_color=BORDER_COLOUR,
            border_color=BORDER_COLOUR,
            border_width=1,
    ...
    def show_status(
        ...
        Thread-safe: background threads (wireless connect, device scan) call this,
        so the actual widget mutation is marshalled onto the Tk thread via after().
        """
        ...
        def _apply():
            ...
            # Re-show the label in case a previous timer hid it.
            try:
                self._status_label.pack(pady=(2, 2))
            except Exception:
                pass
            ...
            self._status_after_id = self.window.after(int(duration * 1000), _clear_status)

        # Queue the update; _update_loop applies it on the Tk thread. Safe to call
        # from any thread because queue.Queue is thread-safe.
        self._ui_queue.put(_apply)

    def force_window_sync(self):
        """Force an immediate dock sync after a layout change."""
        try:
            self.l.force_sync()
        except Exception as e:
    ...
    def _update_loop(self):
        try:
            ...
            # Apply any status updates queued by background threads.
            try:
                while True:
            ...
            # Drive all per-frame backend work (pending connection, device scan,
            # X11 event processing and window-position sync) on the Tk thread.
            self.l.tick()
